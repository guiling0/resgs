...
import { WindowItemDatas } from '../../core/room/room.types';
...
function usecard(room: GameRoom, context: UseCardContext) {
    const from = context.from;
    const req = context.req as GameRequest;
    if (!from) return { selectors: {}, options: {} };
    ...
    if (!context.options.prompt && context.can_use_cards?.length > 0) {
        ...
        if (skill) {
            const prompt = skill.prompt.call(
                skill,
                room,
                from,
                undefined,
    ...
    if (context.can_use_cards.find((v) => v.name === 'wuxiekeji')) {
        (req as any).isWuxie = true;
    }
    const selectors: { [key: string]: ChooseData } = {};
    const can_use_cards = context.can_use_cards ?? [];
    const _card =
        context.card_selector &&
        room.getSelectors(
            context.card_selector.selectorId,
            context.card_selector.context.json
                ? room.toData_Context(context.card_selector.context)
                : (context.card_selector.context as any)
        );
    const _card_key =
        _card?.selectors &&
        Object.keys(_card.selectors).find(
            (v) => _card.selectors[v].type === 'card'
        );
    const cardSelector =
        _card_key && (_card.selectors[_card_key] as ChooseCardData);
    if (cardSelector) {
    ...
    const _vcard_key =
        _card?.selectors &&
        Object.keys(_vcard.selectors).find(
            (v) => _vcard.selectors[v].type === 'vcard'
        );
    const vcardSelector =
        _vcard_key && (_card.selectors[_vcard_key] as ChooseCardData);

    const _target =
        context.target_selector &&
        room.getSelectors(
            context.target_selector.selectorId,
            context.target_selector.context.json
                ? room.toData_Context(context.target_selector.context)
                : (context.target_selector.context as any)
        );
    const _target_key =
        _target?.selectors &&
        Object.keys(_target.selectors).find(
            (v) => _target.selectors[v].type === 'player'
        );
    const targetSelector =
        _target_key && (_target.selectors[_target_key] as ChoosePlayerData);
    if (context.skip_card) {
        context.skip_card.custom.method = context.skip_card.custom.method ?? 1;
        req._use_or_play_vcard = room.createVirtualCardByData(
            context.skip_card,
            true,
            false
        );
        ...
        if (req._carduse_skill) {
            if (req._carduse_skill.target) {
                selectors.target = usecard_target(
                    room,
                    req,
                    context,
                    targetSelector
                ...
                const canuse = from.canUseCard(
                    req._use_or_play_vcard.vdata,
                    undefined,
                    '',
                    targetSelector
                );
    ...
    if (!req._use_or_play_vcard) {
        req._use_or_play_vcard = room.createVirtualCardByNone(
            'sha',
            undefined,
            false
        );
    ...
    if ((!cardSelector && !vcardSelector) || cardSelector) {
        const card: ChooseCardData = { type: 'card' };
        card.step = 1;
        card.count = cardSelector?.count ?? 1;
        card.selectable = cardSelector?.selectable ?? from.getHandCards();
        if (
            lodash.isEmpty(
                lodash.difference(card.selectable, from.getHandCards())
            )
        ) {
            //加入如手牌般使用的卡牌
            const cards = room.cards.filter((v) =>
                room
                    .getStates(StateEffectType.LikeHandToUse, [from, v])
                    .some((s) => s)
            );
        ...
        card.filter = function (item, selected) {
            if (cardSelector) {
                ...
                return !!can_use_cards.find((v) => {
                    ...
                    if (
                        this.player.hasMark('wars.aozhan') &&
                        (name === 'sha' || name === 'shan')
                    ) {
                        (this as any)['__aozhan'] = name;
                        return item.name === name || item.name === 'aozhan';
                    }
                    (this as any)['__aozhan'] = undefined;
        ...
        card.onChange = function (type, item, selected) {
            ...
            if (cardSelector) {
                ...
                let method = 1;
                if (name === 'aozhan') {
                    const canuse = can_use_cards.find(
                        (v) => v.name === 'sha' || v.name === 'shan'
                    );
            ...
            this._carduse_skill = room.getCardUse(
                this._use_or_play_vcard.vdata
            );
            if (this._carduse_skill) {
                const newprompt = this._carduse_skill.prompt.call(
                    this._carduse_skill,
                    room,
                    from,
                ...
                if (this._carduse_skill.target) {
                    this.selectors.target = usecard_target(
                        room,
                        this,
                        context,
                        targetSelector
...
function usecard_target(
    room: GameRoom,
    req: GameRequest,
    context: UseCardContext,
    targetSelector?: ChoosePlayerData
): ChoosePlayerData {
    const selector = req._carduse_skill.target.call(
        req._carduse_skill,
        room,
        context.from,
        req._use_or_play_vcard
    );
    const target: ChoosePlayerData = {
        type: 'player',
    };
    target.step = 9;
    target.excluesDeath =
    ...
    target.canConfirm = function (selected) {
        if (
            ...
            return false;
        if (!(selector.canConfirm?.call(this, selected) ?? true)) return false;
        return true;
    };
    target.filter = function (item, selected) {
        return context.from.canUseCard(
            this._use_or_play_vcard,
            [item],
            '',
            targetSelector,
...
function playcard(room: GameRoom, context: PlayCardContext) {
    const from = context.from;
    const req = context.req as GameRequest;
    if (!from) return { selectors: {}, options: {} };
    if (!context.options) context.options = {};
    ...
    //定义卡牌选择方法
    const card: ChooseCardData | ChooseVCardData = { type: 'card' };
    if (cardSelector) card.type = cardSelector.type;
    card.step = 1;
    card.count = cardSelector?.count ?? 1;
    card.selectable = cardSelector?.selectable ?? from.getHandCards();
    if (
        lodash.isEmpty(lodash.difference(card.selectable, from.getHandCards()))
    ) {
        //加入如手牌般使用的卡牌
        const cards = room.cards.filter((v) =>
            room
                .getStates(StateEffectType.LikeHandToPlay, [from, v])
                .some((s) => s)
        );
        card.selectable.push(...cards);
    }
    ...
    card.onChange = function (type, item, selected) {
        ...
        if (cardSelector) {
            ...
            let name = item.name;
            if (name === 'aozhan') {
                const canuse = can_play_cards.find(
                    (v) => v === 'sha' || v === 'shan'
                );
                name = canuse ?? 'sha';
            }
...
function pindian(room: GameRoom, context: PindianContext) {
    const from = context.from;
    if (!from) return { selectors: {}, options: {} };
    if (!context.options) context.options = {};
    const selectors: { [key: string]: ChooseData } = {};
...
export const base_selectors = sgs.TriggerEffect({
    name: 'base_selectors',
    priorityType: PriorityType.None,
    getSelectors(room, context) {
        return {
            use_card: () => {
                return usecard(room, context as any);
            },
            play_card: () => {
                return playcard(room, context as any);
            },
            watch_hand: () => {
            ...
            watch_general: () => {
                ...
                if (context.req.isSelf) {
                    const his_generals =
                        room.getData<Set<General>>('watch_generals');
                    if (!his_generals) {
                        const set = new Set();
            ...
            choose_pindian: () => {
                return pindian(room, context as any);
            },
            move_filed: () => {
                const pos = context.pos as string;
                    ...
                    datas.forEach((v, i) => {
                        v.items.push({
                            ...
                            card: targets
                                .at(i)
                                ?.equipCards.find(
                                    (c) => c.subtype === CardSubType.Weapon
                                ),
                        });
                    });
                    //防具
                    ...
                    datas.forEach((v, i) => {
                        v.items.push({
                            ...
                            card: targets
                                .at(i)
                                ?.equipCards.find(
                                    (c) => c.subtype === CardSubType.Treasure
                                ),
                        });
                    });
                }
                if (pos.includes('j')) {
                    const judges1 = targets[0]?.judgeCards?.filter(
                        (v) =>
                            !targets[1]?.judgeCards?.find(
                                (v2) => v2.name === v.name
                            )
                    );
                    const judges2 = targets[1]?.judgeCards?.filter(
                        (v) =>
                            !targets[0]?.judgeCards?.find(
                                (v2) => v2.name === v.name
                            )
                    );
                    judges1.forEach((v) => {
            ...
            choose_command_6: () => {
                ...
                let count = 0;
                if (hands.length > 0) count++;
                if (equips.length > 0) count++;
                return {
                    selectors: {
                        card: room.createChooseCard({
                            step: 1,
            ...
            choose_command_89: () => {
                const from = context.from as GamePlayer;
                const to = context.to as GamePlayer;
                return {
                    selectors: {
