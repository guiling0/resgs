...
zuobi.addEffect(
    sgs.TriggerEffect({
        ...
        async cost(room, data, context) {
            ...
            const option = req_result.results.option.result as string[];
            ...
            if (option.includes('zuobi_obtain')) {
                ...
                const res = req.result.results?.card?.result?.at(
                    0
                ) as VirtualCardData;
                if (res && res.name) {
                    const cards = room.cards.filter((v) => v.name === res.name);
                    const card =
                        cards[sgs.utils.randomInt(0, cards.length - 1)];
                    if (card) {
            ...
            if (option.includes('zuobi_change')) {
                ...
                const pos = room.getResult(req, 'option').result as string[];
