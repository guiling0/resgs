...
const wugufengdeng_action1 = sgs.TriggerEffect({
    ...
    trigger: EventTriggers.UseCardReady,
...
export const wugufengdeng = sgs.CardUse({
    name: 'wugufengdeng',
    method: 1,
    ...
    async effect(room, target, data: UseCardEvent) {
        ...
        const s = room.getData<TriggerEffect>('wugufengdeng_action1');
        room.window({
            options: {
                id: windowId,
                title: this.name,
        ...
        const result = room.getResult(req, 'card').result.at(0) as GameCard;
        if (result) {
            result.setLabel({
            ...
            lodash.remove(
                data.data.wugu_cards as GameCard[],
                (c) => c === result
            );
        }
    },
