...
export const huaxin = sgs.General({
    name: 'wars.huaxin',
    kingdom: 'wei',
    hp: 1.5,
    gender: Gender.Male,
...
export const wanggui = sgs.Skill({
...
wanggui.addEffect(
    sgs.TriggerEffect({
        auto_log: 1,
        auto_directline: 1,
        trigger: [EventTriggers.InflictDamaged, EventTriggers.CauseDamaged],
        can_trigger(room, player, data: DamageEvent) {
            if (this.isOwner(player) && this.isOpen()) {
                ...
                if (data.trigger === EventTriggers.InflictDamaged) {
                    return data.to === player;
                }
                if (data.trigger === EventTriggers.CauseDamaged) {
                            ...
                            selectors: {
                                target: room.createChoosePlayer({
                                    ...
                                    filter(item, selected) {
                                        return room.differentAsKingdom(
                                            from,
                                            item
                                        );
                                    },
        ...
        async cost(room, data, context) {
            ...
            if (targets.length === 0) {
                const tos = room.getPlayerByFilter((v) =>
                    room.sameAsKingdom(from, v)
                );
    ...
    sgs.TriggerEffect({
            ...
            if (
                ...
                const use = room.getLastOneHistory(
                    ...
                    (v) =>
                        v !== data &&
                        v.from === data.from &&
                        v.card.color === CardColor.Black &&
                        (v.card.name === 'sha' ||
                            v.card.subtype === CardSubType.InstantScroll),
                    phase
                );
                if (use) return false;
                const use2 = room.getLastOneHistory(
        ...
        async effect(room, data, context) {
            ...
            if (hand < target.hp && hand < 5) {
                ...
                if (draw) {
                    const effect1 = await room.addEffect(
                        'xibing.delay1',
                        target
                    );
            ...
            if (!from.hasNoneOpen() && !target.hasNoneOpen()) {
                await room.chooseYesOrNo(
                    ...
                    async () => {
                        const close_generals: General[] = [];
                        const genearls1 = from.getCanCloseGenerals();
                        let tar_genearl1: General;
                        ...
                        const effect2 = await room.addEffect(
                            'xibing.delay2',
                            from
                        );
...
export const xibing_delay1 = sgs.StateEffect({
    name: 'xibing.delay1',
    mark: ['mark.xibing'],
...
export const xibing_delay2 = sgs.StateEffect({
    name: 'xibing.delay2',
    [StateEffectType.Prohibit_Open](player, generals, reason) {
        const gs = this.getData<General[]>('generals') ?? [];
