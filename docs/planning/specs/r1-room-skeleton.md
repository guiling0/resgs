...
4. **GameClient v0**：snapshot/patches 应用 + event 路由（收到 `kind:'snapshot'|'patches'|'event'` 分发处理）；客户端形态 = **RoomView**（Room 状态树 + 实体查询），按 [ADR 0004](../../decisions/adr/0004-entity-layering.md) 镜像端实体注入 PlayerView（不注入 host），派生值（losshp/距离/手牌上限）经实体查询本地计算；单机模式客户端本地同时运行 RoomEngine（host）
