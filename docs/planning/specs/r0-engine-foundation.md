...
2. **实体**：GameCard / VirtualCard / General / Player / Skill / Effect，全部实现 `MarkHost`（任意实体可存键值标记）；**实体分层按 [ADR 0004](../../decisions/adr/0004-entity-layering.md)**——实体只写一份（状态 + 纯派生 getter + 查询），能力层分 `entity/`（两端）、`registry/`（两端）、`logic/`（PlayerHost 等能力实现，仅权威端运行时存在）、`view/`（PlayerView 镜像端能力实现）
...
- `shared/core/entity/`（GameCard/VirtualCard/General/Player/Skill/Effect/Area + MarkHost，状态 + 派生 getter + 查询，原 query/ 并入；能力方法薄转发 host/view）
- `shared/core/registry/`（sgs 静态注册表：skills/effects/cards/generals）
- `shared/core/logic/`（RoomEngine 雏形 + EventProcess/EventTypes/EventManager + PlayerHost 等能力实现，仅权威端运行时存在）
