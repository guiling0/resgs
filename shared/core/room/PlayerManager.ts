...
export class PlayerManager {
    ...
    createPlayer(
        ...
        this.room.state.game.players.set(playerId, state);
        return player;
    }
    ...
    sort(
        ...
        const maxSeat = this.room.players.length;
    ...
    sortResponse(players: Player[] = this.room.players): Player[] {
        const start = this.room.currentTurn?.player
            ?? this.room.players.find((v) => v.seat === 1);
