...
import type { ChangeMaxHpEventData, RecoverHpEventData } from '../../types/EventTypes';
...
export class RecoverHpEvent extends EventProcess<EventType.RecoverHp> {
    ...
    private _buildTriggers(): void {
        ...
        this.endTriggers = [createTiming(TimingName.RecoverHpEnd)];
    }
    ...
    private async _onRecoverHpAfter(_room: Room, _data: RecoverHpEventData): Promise<void> {
        const p = this.player;
        p.hp = p.hp + this.number;
        this.room.logger.info(
            `recover player=${p.playerId} +${this.number} → hp=${p.hp}`,
            { roomId: this.room.roomId, playerId: this.player.playerId, event: `RecoverHp:${this.id}._onRecoverHpAfter` },
        );
...
export class ChangeMaxHpEvent extends EventProcess<EventType.ChangeMaxHp> {
    ...
    private _buildTriggers(): void {
        ...
        this.endTriggers = [createTiming(TimingName.ChangeMaxHpEnd)];
    }
    ...
    private async _onChangeMaxHpAfter(_room: Room, _data: ChangeMaxHpEventData): Promise<void> {
        const p = this.player;
        ...
        this.room.logger.info(
            ...
            { roomId: this.room.roomId, playerId: p.playerId, event: `ChangeMaxHp:${this.id}._onChangeMaxHpAfter` },
        );
