...
async function test_initialState(): Promise<void> {
    ...
    assert(room.game.turnCount === 0, 'state.turnCount=0');
    assert(room.game.roundCount === 0, 'state.roundCount=0');    assert(!room.isGaming, '未开始游戏');
...
async function test_stateDefaults(): Promise<void> {
    ...
    assert(room.game.turnCount === 0, 'state.turnCount=0');
    assert(room.game.roundCount === 0, 'state.roundCount=0');}
...
async function test_modeNotFound(): Promise<void> {
    ...
    assert(room.game.roundCount === 0, '未进入主循环');
}
