...
| **M4 + C4** | 锦囊/装备 + StateEffect 接线 + 客户端适配 | headless 8 人对战 + 客户端完整操作 |
| **M5 + C5** | AI + 自动对战 + 客户端适配 | N 局无异常 + 动画音效 |
| **M6** | 27 武将技能流水线 + 客户端适配 | 武将全可用 |
| **M7 + C6** | 联机：GameRoom 修复 + 大厅/房间/聊天/观战 | 真实联机一局 |
| **M8** | 录像/旁观 + 武将 Spine 动效 + 录像回放 | 完整游戏 |
