...
export const lianjunshengyan_choose = sgs.TriggerEffect({
    name: 'lianjunshengyan_choose',
    getSelectors(room, context) {
        return {
            choose: () => {
                const from = context.from;
                const length = context.length as number;
...
export const lianjunshengyan = sgs.CardUse({
    name: 'lianjunshengyan',
    method: 1,
    trigger: EventTriggers.PlayPhaseProceeding,
    effects: [lianjunshengyan_choose.name],
    ...
    async effect(room, target, data: UseCardEvent) {
        ...
        if (current.target === from) {
            ...
            const s = room.getData<TriggerEffect>('lianjunshengyan_choose');
