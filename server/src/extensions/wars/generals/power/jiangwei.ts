...
import { arraycall_queue, reduce_yinyangyu } from '../../rules';

export const jiangwei = sgs.General({
    name: 'wars.jiangwei',
    kingdom: 'shu',
    hp: 2,
    gender: Gender.Male,
...
export const tiaoxin = sgs.Skill({
    ...
    sgs.TriggerEffect({
        ...
        can_trigger(room, player, data: PhaseEvent) {
            if (this.isOwner(player) && data.isOwner(player)) {
                const guanxing = room.skills.find(
                    (v) =>
                        v !== this.skill &&
...
export const tianfu = sgs.Skill({
    name: 'wars.jiangwei.tianfu',
});

tianfu.addEffect(
    sgs.StateEffect({
        tag: [SkillTag.Head, SkillTag.Array_Quene],
        regard_skill(room, player, data) {
            if (this.isOwner(player)) {
...
tianfu.addEffect(sgs.copy(arraycall_queue));
...
jiangwei.addSkill('#wars.jiangwei.kanpo');
jiangwei.addSkill(yizhi);
jiangwei.addSkill('#wars.jiangwei.guanxing');
