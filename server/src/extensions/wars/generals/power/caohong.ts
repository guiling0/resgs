...
import { arraycall_queue } from '../../rules';

export const caohong = sgs.General({
    name: 'wars.caohong',
    kingdom: 'wei',
    hp: 2,
    gender: Gender.Male,
...
huyuan.addEffect(
    sgs.TriggerEffect({
        ...
        async effect(room, data, context) {
            ...
            const players = room.getPlayerByFilter(
                (v) => target.distanceTo(v) === 1 && v.hasCardsInArea()
            );
...
heyi.addEffect(sgs.copy(arraycall_queue));

export const feiying = sgs.Skill({
...
caohong.addSkill('#wars.caohong.feiying');
