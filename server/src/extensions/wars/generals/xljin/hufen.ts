...
export const hufen = sgs.General({
    name: 'xl.hufen',
    kingdom: 'jin',
    hp: 2,
    gender: Gender.Male,
...
export const jiesha_delay = sgs.TriggerEffect({
    name: 'jiesha.delay',
    audio: [],
    ...
    can_trigger(room, player, data: DamageEvent) {
        return (
            ...
            this.getData('count') &&
            this.getData('target') &&
            this.getData<GamePlayer>('target').alive
        );
    },
    getSelectors(room, context) {
        return {
...
export const jiesha_delay_draw = sgs.TriggerEffect({
...
export const jiesha_invalids = sgs.StateEffect({
    ...
    [StateEffectType.Skill_Invalidity](effect) {
        if (
            ...
            const markkey = this.getData<string>('mark') ?? 'jiesha.delay';
    ...
    lifecycle: [
        {
            ...
            async on_exec(room, data) {
                if (this.getData('turn') === data) {
                    await this.removeSelf();
                    const markkey =
                        this.getData<string>('mark') ?? 'jiesha.delay';
...
hufen.addSkill(jiesha);

sgs.loadTranslation({
    [jiesha_delay.name]: '截杀',
