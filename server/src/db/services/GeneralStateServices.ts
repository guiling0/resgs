...
import { GeneralState, WarsState } from '../models/GeneralState';

export class GeneralStatsService {
    private readonly collection =
        getDatabase().collection<GeneralState>('generals');

    async processGameResult(matchResult: MatchResult) {
    ...
    private async processWarsGameResult(matchResult: MatchResult) {
    ...
    private async calculate(matchResult: MatchResult) {
        const { mode, generals } = matchResult;
                ...
                if (state) {
                    ...
                    if (totalPicks > 0) {
                        update[`statsByMode.${mode}.winRate`] =
                            this.roundToTwoDecimal(
                                (state.overallWins || 0) / totalPicks
                            );
                    }
                    //asHead
                    ...
                    if (asHead) {
                        const totalPicks =
                            (asHead.initialPicks || 0) +
                            (asHead.changePicks || 0);
                        if (state.offered && state.offered > 0) {
                            update[`statsByMode.${mode}.asHead.pickRate`] =
                                this.roundToTwoDecimal(
                                    (asHead.initialPicks || 0) / state.offered
                                );
                        }
                        if (asHead.initialPicks && asHead.initialPicks > 0) {
                            update[
                                `statsByMode.${mode}.asHead.initialPickWinRate`
                            ] = this.roundToTwoDecimal(
                                (asHead.initialPickWins || 0) /
                                    asHead.initialPicks
                            );
                        }
                        if (totalPicks > 0) {
                            update[`statsByMode.${mode}.asHead.winRate`] =
                                this.roundToTwoDecimal(
                                    (asHead.overallWins || 0) / totalPicks
                                );
                        }
                    }
                    //asHead
                    ...
                    if (asDeputy) {
                        ...
                        if (state.offered && state.offered > 0) {
                            update[`statsByMode.${mode}.asDeputy.pickRate`] =
                                this.roundToTwoDecimal(
                                    (asDeputy.initialPicks || 0) / state.offered
                                );
                        }
                        if (
                            ...
                            update[
                                `statsByMode.${mode}.asDeputy.initialPickWinRate`
                            ] = this.roundToTwoDecimal(
                                (asDeputy.initialPickWins || 0) /
                                    asDeputy.initialPicks
                            );
                        }
                        if (totalPicks > 0) {
                            update[`statsByMode.${mode}.asDeputy.winRate`] =
                                this.roundToTwoDecimal(
                                    (asDeputy.overallWins || 0) / totalPicks
                                );
                        }
                    }
                }
