...
exports.qingke = exports.xisui = exports.zhangze = void 0;
exports.zhangze = sgs.General({
    name: 'xl.zhangze',
    kingdom: 'qun',
    hp: 2,
...
exports.xisui = sgs.Skill({
    name: 'xl.zhangze.xisui',
});
exports.xisui.addEffect(sgs.TriggerEffect({
    auto_log: 1,
    auto_directline: 1,
    ...
    async effect(room, data, context) {
        ...
        if (result.includes('xisui.drop')) {
            ...
            const drops = target
                .getHandCards()
                .filter((card) => upcards.find((v) => v.suit === card.suit));
...
exports.qingke = sgs.Skill({
    name: 'xl.zhangze.qingke',
});
exports.qingke.addEffect(sgs.TriggerEffect({
    auto_log: 1,
    trigger: "MoveCardAfter1" /* EventTriggers.MoveCardAfter1 */,
    can_trigger(room, player, data) {
        return (this.isOwner(player) &&
            data.has_filter((d, c) => (d.fromArea === player.equipArea ||
    ...
    async cost(room, data, context) {
        const { from } = context;
        const cards = data.getCards((d, c) => (d.fromArea === from.equipArea ||
            d.fromArea === from.handArea) &&
            d.toArea.player &&
            d.toArea.player !== from);
        cards.forEach((v) => v.turnTo(1 /* CardPut.Up */));
        await room.showCards({
            player: from,
            cards,
            source: data,
...
exports.qingke.addEffect(sgs.TriggerEffect({
    ...
    can_trigger(room, player, data) {
        return (data.has_lose(player, 'h') &&
            player.getHandCards().filter((v) => v.put === 1 /* CardPut.Up */)
                .length === 0 &&
            ...
            async on_exec(room, data) {
                ...
                if (data.is(sgs.DataType.MoveCardEvent)) {
                    ...
                    data.move_datas.forEach((v) => {
                        if (v.fromArea.type === 91 /* AreaType.Hand */ &&
                            v.cards.find((c) => c.put === 1 /* CardPut.Up */)) {
...
exports.zhangze.addSkill(exports.xisui);
exports.zhangze.addSkill(exports.qingke);
sgs.loadTranslation({
