...
exports.liedu = exports.shanzheng = exports.jiananfeng = void 0;
exports.jiananfeng = sgs.General({
...
exports.shanzheng = sgs.Skill({
    name: 'wars.jiananfeng.shanzheng',
});
exports.shanzheng.addEffect(sgs.TriggerEffect({
    auto_log: 1,
    auto_directline: 1,
                ...
                return {
                    selectors: {
                        player: room.createChoosePlayer({
                            ...
                            count: 1,
                            filter(item, selected) {
                                return (from !== item && !targets.includes(item));
                            },
                        }),
                    },
                    options: {
    ...
    async cost(room, data, context) {
        ...
        const reqs = await room.doRequestAll([from, ...targets].map((v) => {
            return {
                player: v,
                get_selectors: {
    ...
    async effect(room, data, context) {
        ...
        let black = 0;
        cards.forEach((v) => {
            if (v.color == 1 /* CardColor.Red */) {
                red++;
            }
            if (v.color === 2 /* CardColor.Black */) {
                black++;
            }
        });
        if (red > black) {
        ...
        if (black > red) {
            await room.obtainCards({
                player: from,
                cards: cards.filter((v) => v.color === 2 /* CardColor.Black */),
                source: data,
                reason: this.name,
            });
            room.broadcast({
...
exports.liedu = sgs.Skill({
    name: 'wars.jiananfeng.liedu',
});
exports.liedu.addEffect(sgs.TriggerEffect({
    tag: [1 /* SkillTag.Lock */],
    ...
    can_trigger(room, player, data) {
        return (this.isOwner(player) &&
            data.from === player &&
            !data.data[this.id] &&
            data.targetList.some((v) => v.target !== player &&
                (v.target.gender === 2 /* Gender.Female */ ||
    ...
    async cost(room, data, context) {
        ...
        return await data.targetCantResponse(data.targetList.map((v) => {
            if (v.target !== from &&
                (v.target.gender === 2 /* Gender.Female */ ||
                    v.target.getHandCards().length >
...
exports.jiananfeng.addSkill(exports.shanzheng);
exports.jiananfeng.addSkill(exports.liedu);
