...
exports.DieEvent = exports.DyingEvent = void 0;
const event_1 = require("../event");
const event_damage_1 = require("./event.damage");
const event_move_1 = require("./event.move");
/**
 * 濒死事件
 */
class DyingEvent extends event_1.EventProcess {
    ...
    async init() {
        super.init();
        this.eventTriggers = [
            "EntryDying" /* EventTriggers.EntryDying */,
            "EntryDyinged" /* EventTriggers.EntryDyinged */,
            "Dying" /* EventTriggers.Dying */,
        ];
        this.endTriggers = ["DyingEnd" /* EventTriggers.DyingEnd */];
    ...
    async [`${"Dying" /* EventTriggers.Dying */}_Before`]() {
        this.triggerNot = true;
        await this.room.trigger("Dying" /* EventTriggers.Dying */, this, undefined, () => {
            return this.check_event();
        });
    }
    async [`${"Dying" /* EventTriggers.Dying */}_After`]() {
        this.player.setProperty('indying', -this.eventId);
    ...
    getDamage() {
        if (this.source instanceof event_damage_1.ReduceHpEvent &&
            this.reason === 'dying_reducehp') {
            return this.source.getDamage();
        }
    }
}
exports.DyingEvent = DyingEvent;
/**
 * 死亡事件
 */
class DieEvent extends event_1.EventProcess {
    static async exec(room, data) {
    ...
    async init() {
        await super.init();
        this.eventTriggers = [
            "BeforeDeath" /* EventTriggers.BeforeDeath */,
            "ConfirmRole" /* EventTriggers.ConfirmRole */,
            "Death" /* EventTriggers.Death */,
            "Deathed" /* EventTriggers.Deathed */,
        ];
        this.endTriggers = ["DieEnd" /* EventTriggers.DieEnd */];
        this.player.setProperty('headOpen', true);
        this.player.setProperty('deputyOpen', true);
        this.room.insertHistory(this);
    }
    async [`${"BeforeDeath" /* EventTriggers.BeforeDeath */}_After`]() {
    ...
    async [`${"Death" /* EventTriggers.Death */}_After`]() {
        //将手牌和装备牌弃置
        const drop = this.room.createEventData(event_move_1.DropCardsData, {
            player: this.player,
            cards: [
        ...
        //判定区，武将牌上，武将牌旁置入弃牌堆
        const putto = this.room.createEventData(event_move_1.PutToCardsData, {
            player: this.player,
            cards: [
        ...
        if (this.player.rest === 0) {
            ...
            const up_generals = this.player.upArea.generals.filter((v) => !v.hasMark('$not_die_handle'));
            this.player.upArea.remove(up_generals);
            const side_generals = this.player.sideArea.generals.filter((v) => !v.hasMark('$not_die_handle'));
    ...
    async [`${"Deathed" /* EventTriggers.Deathed */}_After`]() {
        if (this.room.mode.name === 'wars') {
            if (this.player.head?.isLord()) {
                ...
                this.room.players.forEach((v) => {
                    if (v.kingdom === this.player.head.kingdom &&
                        v !== this.player) {
    ...
    async [`${"DieEnd" /* EventTriggers.DieEnd */}_After`]() {
        if (this.player.death && this.player.rest === 0) {
            //失去所有技能
            while (true) {
                const skill = this.room.skills.find((v) => v.player === this.player);
                if (skill) {
